...
/usr/local/silo/agent/drivers/ollo-zigbee/bin/setuplprfcape_reva1.sh
...
do
	...
	modelId="$(/usr/local/silo/agent/drivers/ollo-zigbee/bin/zbFwStart.linux.arm.bin $zbCdcDev$zbDevCdcIdx)"
...
if [[ -z "$zbDev" ]]; then
	zbttyDev=/dev/ttyO4
        ...
        modelId="$(/usr/local/silo/agent/drivers/ollo-zigbee/bin/zbFwStart.linux.arm.bin $zbttyDev)"
...
if [[ $modelId =~ "ZBGW" ]] ; then
	...
	/usr/local/silo/agent/drivers/ollo-zigbee/bin/zbGateway.linux.arm.bin $zbDev > $zbServerLog 2>&1
else
	if [[ $modelId =~ "TI SampleBridge" ]] ; then
        	echo "Starting zllGateway with $zbDev" >> $zbStartUpLog
        	/usr/local/silo/agent/drivers/ollo-zigbee/bin/zllGateway.linux.arm.bin $zbDev > $zbServerLog 2>&1
	else
